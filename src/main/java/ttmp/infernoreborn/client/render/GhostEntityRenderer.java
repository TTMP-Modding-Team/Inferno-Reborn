...
public class GhostEntityRenderer extends EntityRenderer<GhostEntity>{
	...
	public GhostEntityRenderer(EntityRendererManager manager){
		super(manager);
	}

	@Override
	protected int getBlockLightLevel(GhostEntity e, BlockPos p){
		return 15;
	}

	@Override
	public void render(GhostEntity entity, float entityYaw, float partialTicks, MatrixStack stack, IRenderTypeBuffer typeBuffer, int packedLight){
		stack.pushPose();
		stack.scale(2, 2, 2);
		...
		vertex(buffer, pose, normal, packedLight, 0, 0, 0, 1, 1);
		vertex(buffer, pose, normal, packedLight, 1, 0, 1, 1, 1);
		vertex(buffer, pose, normal, packedLight, 1, 1, 1, 0, 1);
		vertex(buffer, pose, normal, packedLight, 0, 1, 0, 0, 1);
		stack.popPose();
	...
	@Override public ResourceLocation getTextureLocation(GhostEntity e){
