		...
		if(e!=null){
			...
			if(h!=null){
				for(EssenceType type : EssenceType.values()){
					...
					if(amount>=9*9){
						generatedLoot.add(new ItemStack(type.getGreaterCrystalItem(), amount/9*9));
						amount %= 9*9;
					}
					if(amount>=9){
						generatedLoot.add(new ItemStack(type.getCrystalItem(), amount/9));
						amount %= 9;
					}
					if(amount>=1){
						generatedLoot.add(new ItemStack(type.getShardItem(), amount));
					}
				}
			}
		}
