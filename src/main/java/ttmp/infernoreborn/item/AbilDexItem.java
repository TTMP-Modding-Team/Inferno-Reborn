...
public class AbilDexItem extends Item{
		...
		if(player instanceof ServerPlayerEntity){
			if(player.isCrouching()&&hasEssenceHolder(stack)){
				player.openMenu(new EssenceHolderContainerProvider(
						new TranslationTextComponent("container.infernoreborn.abildex"),
						player,
						hand));
			}else{
	...
	@Override public void appendHoverText(ItemStack stack, @Nullable World world, List<ITextComponent> text, ITooltipFlag flag){
		...
		text.add(new TranslationTextComponent("item.infernoreborn.abildex.desc.essence_holder").setStyle(Style.EMPTY.applyFormat(TextFormatting.YELLOW)));
